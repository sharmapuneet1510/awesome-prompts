...
def test_copilot_exporter_hook_output_dir(tmp_path):
    from tools.exporter import CopilotExporter
    assert CopilotExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".github" / "hooks"
...
def test_cursor_exporter_hook_output_dir(tmp_path):
    from tools.exporter import CursorExporter
    assert CursorExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".cursor" / "rules" / "hooks"
...
def test_windsurf_exporter_hook_output_dir(tmp_path):
    from tools.exporter import WindsurfExporter
    assert WindsurfExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".windsurf" / "rules" / "hooks"
...
def test_gemini_exporter_hook_output_dir(tmp_path):
    from tools.exporter import GeminiExporter
    assert GeminiExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".gemini" / "hooks"
...
def test_continue_exporter_hook_output_dir(tmp_path):
    from tools.exporter import ContinueExporter
    assert ContinueExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".continue" / "hooks"
...
def test_openai_exporter_hook_output_dir(tmp_path):
    from tools.exporter import OpenAIExporter
    assert OpenAIExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / "tools" / "output" / "openai" / "hooks"
...
def test_aider_exporter_hook_output_dir(tmp_path):
    from tools.exporter import AiderExporter
    assert AiderExporter(repo_root=tmp_path).hook_output_dir() == tmp_path / ".aider" / "hooks"
...
def test_generate_claude_settings(tmp_path):
    """Test ConfigGenerator.generate_claude_settings() creates valid settings."""
    from tools.config_generator import ConfigGenerator
    from tools.exporter import HookFile

    # Create a mock hook
    hook_path = tmp_path / "test_hook.sh"
...
def test_generate_copilot_config(tmp_path):
    """Test ConfigGenerator.generate_copilot_config() creates valid YAML."""
    from tools.config_generator import ConfigGenerator
    from tools.exporter import HookFile
...
""",
    ...
    config_gen = ConfigGenerator(repo_root)
    config_yaml = config_gen.generate_copilot_config([hook])

    # Verify YAML structure
    assert "version: 1" in config_yaml
    assert "hooks:" in config_yaml
