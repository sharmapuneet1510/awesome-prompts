...
@pytest.mark.parametrize(
    ...
    [
        ("DEFEATED", [{"severity": "CRITICAL", "confidence": "HIGH_CONFIDENCE", "counterexample": "steps"}], True),
        ("DEFEATED", [{"severity": "CRITICAL", "confidence": "SPECULATIVE", "counterexample": "steps"}], False),  # speculative cannot defeat
        ("DEFEATED", [{"severity": "HIGH", "confidence": "CONFIRMED", "counterexample": "none"}], False),  # no counterexample
        ("CHALLENGED", [{"severity": "HIGH", "confidence": "CONFIRMED", "counterexample": "none"}], True),
        ("CHALLENGED", [{"severity": "MEDIUM", "confidence": "PLAUSIBLE", "counterexample": "none"}] * 2, True),
        ("CHALLENGED", [{"severity": "LOW", "confidence": "CONFIRMED", "counterexample": "none"}], False),
        ("SURVIVED WITH CONDITIONS", [{"severity": "CRITICAL", "confidence": "SPECULATIVE", "counterexample": "none"}], True),  # capped
        ("SURVIVED WITH CONDITIONS", [{"severity": "HIGH", "confidence": "PLAUSIBLE", "counterexample": "none"}], False),
        ("SURVIVED", [], True),
        ("SURVIVED", [{"severity": "LOW", "confidence": "CONFIRMED", "counterexample": "none"}], False),  # a finding means it is not a clean survive
        ("SURVIVED", [{"severity": "CRITICAL", "confidence": "CONFIRMED", "counterexample": "steps"}], False),
        ("INSUFFICIENT EVIDENCE", [], True),
        ("INSUFFICIENT EVIDENCE", [{"severity": "CRITICAL", "confidence": "CONFIRMED", "counterexample": "steps"}], False),
    ],
)
def test_the_verdict_rules_behave_as_documented(verdict, findings, expected):
    assert c.permitted(verdict, findings) is expected
````
...
Expected: **PASS** — `310 passed` with the same `4 failed` and `41 errors` as the baseline: 233 + 77 = 310, and **no new failure**.
