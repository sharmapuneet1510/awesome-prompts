...
- **The final whole-branch review** found that the loop guard counted by `target`, which changes on every re-review (each PR or review has a new id), so it could never fire. Reports now carry a stable `change` header key (the Jira key, otherwise the PR or release id) and the gates count two consecutive **blocking** results (`DEFEATED`, `CHALLENGED` or `INSUFFICIENT EVIDENCE`) by it. The same wave: the report header pins a `target.type` enumeration; the `findings` counts exclude `DISPROVEN` findings (the SURVIVED example now records one disproven challenge); the release persona is `quality:observe` plus `orchestrator:risk` (the earlier `implementer:pipeline` builds pipelines, it does not assess readiness); the config template covers `architecture_change` and `regulatory_change` and says that `enabled: false` also refuses a manual run; the archive README's stale counts were corrected (the docs group is now 62 edits over 17 files); and the workflow example uses `PR-1839` throughout. The traceability test's docstring now says it ties declarations, not behaviour.
- **The scoped re-review of that wave** found that the `change` fallback (the PR or release id) still changes on every re-review for a project without Jira keys; it is now the id of the **first** target in the chain, and a re-review inherits the `change` of the report it re-challenges. The skill's sequence rule now says per year, the archive README count is asserted by the registration test, and the spec records the controller rulings as D8-D10.
...
| 17 existing documents | registration and counts | 5 |
...
EDITS_DOCS = [
    ...
    ("docs/99-archive/README.md", "**5 agents, 42 functions, 36 skills**", "**5 agents, 43 functions, 37 skills**"),
...
Expected: **PASS** — `nemesis_register docs: 62 edits in 17 files`
...
Expected: **PASS** — `341 passed` with the same `4 failed` and `41 errors` as the baseline: 233 + 108 = 341, and **no new failure**.
