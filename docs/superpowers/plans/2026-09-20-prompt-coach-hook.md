...
**Spec:** `docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md` (approved 2026-09-20; amended by Task 0). The plan argues from the spec; read both.
...
- Source lives in `tools/prompt_coach/`, not `hooks/`; the wizard is the only installer (D4).
- Commit messages are plain, with **no co-author trailer** (the user's standing preference). Work stays on branch `feat/prompt-coach`; nothing is pushed without the user's say-so.
...
## Proposed spec amendments (need the user's approval)
...
|---|---|---|
| A1 | `clarify` and `refine` only on the **first prompt of a session** (tracked by `session_id`). | The existing analyzer marked **11 of 12** ordinary mid-session replies as `skip`: "yes", "continue", "commit and push", "approve", "thanks", "run the tests", "go ahead", … As specified, the Coach would have told the user "too vague to act on" for almost every reply they type. |
...
|---|---|---|
| `tools/prompt_coach/__init__.py` | package marker and `__version__` | 1 |
| `tools/prompt_coach/errors.py` | `ModelUnavailable` | 1 |
| `tools/prompt_coach/defaults.py` | bake-off outputs (recommended model, size, budget) | 1, 4 |
| `tools/prompt_coach/config.py` | defaults, validated load, loopback check | 1 |
| `tools/prompt_coach/state.py` | cooldown, once-a-day notice, seen sessions, block override | 1 |
| `tools/prompt_coach/heuristics.py` | tier 1: context signals and the `google` decision | 2 |
| `tools/prompt_coach/decide.py` | pure verdict logic and guardrails | 2 |
| `tools/prompt_coach/output.py` | Decision → hook JSON, with caps and sanitizing | 2 |
| `tools/prompt_coach/ollama_client.py` | loopback-only, proxy-free model client | 3 |
| `tools/prompt_coach/eval/` | 60-prompt labeled set, bake-off runner, results | 4 |
| `tools/prompt_coach/hook.py`, `launcher.py` | entry point (always exit 0) and the installed stub | 5 |
| `tools/prompt_coach/settings_merge.py` | safe merge, backup, atomic write, diff | 6 |
| `tools/prompt_coach/setup.py` | the opt-in wizard | 6 |
| `tools/interactive_exporter.py` | one opt-in question (modify) | 7 |
| `docs/03-guides/prompt-coach.md` | user guide | 8 |
| `tests/prompt_coach/` | 13 test modules, 267 tests | all |
...
**Files:**
- Modify: `docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md`
...
Do this only after the user has approved this plan, which includes the five amendments listed above. Nine edits, each to text that occurs exactly once.

In `docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md`, replace:
...
````text
| D8 | Default model chosen by measurement; heuristics-only is an allowed outcome | **DECISION** (accepted, user, 2026-09-20) |
| D9 | Conversation awareness: `clarify` and `refine` only on a session's first prompt; tier 1 decides `google` only (Amendments A1–A3) | **DECISION** (accepted, user, with the plan) |
````
...
```bash
git add docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md
git commit -m "docs: amend Prompt Coach spec (conversation awareness, min_words, off-ramp rule)"
```
...
**Files:**
- Create: `tools/prompt_coach/__init__.py`, `errors.py`, `defaults.py`, `config.py`, `state.py`
- Create: `tests/prompt_coach/__init__.py`, `test_config.py`, `test_state.py`

**Interfaces:**
- Produces:
  - `prompt_coach.errors.ModelUnavailable(Exception)`
  - `prompt_coach.defaults`: `RECOMMENDED_MODEL: Optional[str] = None`, `RECOMMENDED_MODEL_SIZE_GB: Optional[float] = None`, `RECOMMENDED_BUDGET_MS: int = 2500`
  - `prompt_coach.config`: `DEFAULTS: Dict[str, Any]`, `load_config(path: str) -> Dict[str, Any]` (never raises), `host_only(host: str) -> str`, `is_loopback(host: str) -> bool`
  - `prompt_coach.state.State(path: str, clock=time.time)` with `in_cooldown() -> bool`, `start_cooldown(seconds: float) -> None`, `notice_due(kind: str, every_s: float = 86400) -> bool`, `register_session(session_id: str) -> bool`, `remember_block(prompt: str) -> None`, `consume_override(prompt: str, window_s: float) -> bool`

- [ ] **Step 1: Create the package skeleton**

The import path works because `tests/conftest.py` already puts `tools/` on `sys.path`; the installed hook gets the same layout (`prompt_coach/` beside `token_optimizer/`).

Run: `mkdir -p tools/prompt_coach tests/prompt_coach && touch tests/prompt_coach/__init__.py`

Expected: no output.

**`tools/prompt_coach/__init__.py`**

````python
"""Prompt Coach: an optional Claude Code UserPromptSubmit hook. See docs/03-guides/prompt-coach.md."""
...
**`tests/prompt_coach/test_config.py`**
...
from prompt_coach.config import DEFAULTS, host_only, is_loopback, load_config
...
Run: `python3 -m pytest tests/prompt_coach/test_config.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.config'`

- [ ] **Step 3: Config: implementation**

**`tools/prompt_coach/errors.py`**

````python
"""Exceptions shared by the Prompt Coach modules."""
...
**`tools/prompt_coach/defaults.py`**
...
**`tools/prompt_coach/config.py`**

````python
"""Load and validate the Prompt Coach configuration. Loading never raises."""
import copy
import ipaddress
import json
...
**`tests/prompt_coach/test_state.py`**

````python
import os

from prompt_coach.state import State
...
Run: `python3 -m pytest tests/prompt_coach/test_state.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.state'`

- [ ] **Step 5: State: implementation**

**`tools/prompt_coach/state.py`**
...
Run: `python3 -m pytest tests/prompt_coach -q`

Expected: **PASS** — `43 passed`

```bash
git add tools/prompt_coach/__init__.py tools/prompt_coach/errors.py tools/prompt_coach/defaults.py tools/prompt_coach/config.py tools/prompt_coach/state.py tests/prompt_coach/__init__.py tests/prompt_coach/test_config.py tests/prompt_coach/test_state.py
git commit -m "feat(prompt-coach): add package scaffold, config loading, and state"
```
...
**Files:**
- Create: `tools/prompt_coach/heuristics.py`, `decide.py`, `output.py`
- Create: `tests/prompt_coach/test_heuristics.py`, `test_decide.py`, `test_output.py`
...
**`tests/prompt_coach/test_heuristics.py`**

````python
import pytest

from prompt_coach.heuristics import context_signals, tier1
...
Run: `python3 -m pytest tests/prompt_coach/test_heuristics.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.heuristics'`

- [ ] **Step 2: Heuristics: implementation**

**`tools/prompt_coach/heuristics.py`**
...
**`tests/prompt_coach/test_decide.py`**
...
from prompt_coach.config import DEFAULTS
from prompt_coach.decide import PASS, decide, search_query, should_skip
from prompt_coach.errors import ModelUnavailable
...
Run: `python3 -m pytest tests/prompt_coach/test_decide.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.decide'`

- [ ] **Step 4: Verdict logic: implementation**

**`tools/prompt_coach/decide.py`**
...
**`tests/prompt_coach/test_output.py`**

````python
import copy

from prompt_coach.config import DEFAULTS
from prompt_coach.decide import PASS, Decision
from prompt_coach.output import MAX_CONTEXT, build_output, degraded_notice, sanitize
...
def test_google_advises_the_user_only():
    ...
    assert out == {"systemMessage": 'Prompt Coach: quick lookup — try Google: "python reverse list"'}
...
Run: `python3 -m pytest tests/prompt_coach/test_output.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.output'`

- [ ] **Step 6: Output: implementation**

**`tools/prompt_coach/output.py`**
...
def build_output(decision: Decision, cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if decision.verdict == "google":
        message = 'Prompt Coach: quick lookup — try Google: "%s"' % sanitize(decision.google_query, MAX_QUERY)
        if cfg["mode"] == "block":
    ...
    if decision.verdict == "clarify":
        ...
        out = _context(
            "Prompt Coach (advisory): the request may be under-specified. Possible gaps: %s. "
            "Ask the user to clarify before acting." % gaps
        )
        out["systemMessage"] = "Prompt Coach: this may be too vague to act on. Missing: %s" % gaps
        return out

    if decision.verdict == "refine":
        refined = sanitize(decision.refined, MAX_REFINED)
        return _context(
            "Prompt Coach (advisory, from a small local model; the user's original prompt is "
...
def degraded_notice() -> Dict[str, Any]:
    return {"systemMessage": "Prompt Coach: the local model is unavailable, so it is running heuristics-only."}
````
...
```bash
git add tools/prompt_coach/heuristics.py tools/prompt_coach/decide.py tools/prompt_coach/output.py tests/prompt_coach/test_heuristics.py tests/prompt_coach/test_decide.py tests/prompt_coach/test_output.py
git commit -m "feat(prompt-coach): add heuristics, verdict logic, and hook output"
```
...
**Files:**
- Create: `tools/prompt_coach/ollama_client.py`
- Create: `tests/prompt_coach/fake_ollama.py`, `tests/prompt_coach/test_ollama_client.py`
...
**`tests/prompt_coach/fake_ollama.py`**
...
**`tests/prompt_coach/test_ollama_client.py`**
...
from prompt_coach.config import DEFAULTS
from prompt_coach.errors import ModelUnavailable
from prompt_coach.ollama_client import SCHEMA, build_request, classify, parse_reply
...
Run: `python3 -m pytest tests/prompt_coach/test_ollama_client.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.ollama_client'`
...
**`tools/prompt_coach/ollama_client.py`**
...
```bash
git add tools/prompt_coach/ollama_client.py tests/prompt_coach/fake_ollama.py tests/prompt_coach/test_ollama_client.py
git commit -m "feat(prompt-coach): add loopback-only Ollama client with fake server tests"
```
...
**Files:**
- Create: `tools/prompt_coach/eval/__init__.py`, `prompts.jsonl`, `run_eval.py`
- Create: `tests/prompt_coach/test_eval.py`
- Modify: `tools/prompt_coach/defaults.py` (from the results)
- Modify: `docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md` (append the results)
...
**`tests/prompt_coach/test_eval.py`**
...
from prompt_coach.config import DEFAULTS
from prompt_coach.decide import should_skip
from prompt_coach.errors import ModelUnavailable
from prompt_coach.eval import run_eval as ev
...
Run: `python3 -m pytest tests/prompt_coach/test_eval.py -q`

Expected: **FAIL** — `No module named 'prompt_coach.eval'`
...
Run: `mkdir -p tools/prompt_coach/eval tools/prompt_coach/eval/results && touch tools/prompt_coach/eval/__init__.py`

Expected: 

**`tools/prompt_coach/eval/prompts.jsonl`**
...
**`tools/prompt_coach/eval/run_eval.py`**

````python
"""Bake-off runner: score the whole pipeline (tier 1 + optional model) on the labeled eval set.

    python3 tools/prompt_coach/eval/run_eval.py --baseline
    python3 tools/prompt_coach/eval/run_eval.py --model <ollama-model> --json out.json
    python3 tools/prompt_coach/eval/run_eval.py --table baseline.json out.json
...
from prompt_coach import ollama_client  # noqa: E402
from prompt_coach.config import DEFAULTS  # noqa: E402
from prompt_coach.decide import decide  # noqa: E402
from prompt_coach.errors import ModelUnavailable  # noqa: E402
...
def measure_startup_ms(runs: int = 5) -> float:
    ...
    code = "from prompt_coach.decide import decide"
    samples = []
    for _ in range(runs):
        started = time.perf_counter()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Score the Prompt Coach pipeline on the eval set.")
...
```bash
git add tools/prompt_coach/eval/__init__.py tools/prompt_coach/eval/prompts.jsonl tools/prompt_coach/eval/run_eval.py tests/prompt_coach/test_eval.py
git commit -m "feat(prompt-coach): add labeled eval set and bake-off runner"
```

- [ ] **Step 3: GATE A — the user reviews the eval labels**

> **STOP — needs the user.** Show the user `tools/prompt_coach/eval/prompts.jsonl` (60 lines: `prompt`, `label`, `guardrail`). The labels are RULE 12 PROPOSALS until the user approves them; they may add, remove, or relabel prompts. If they change any label, re-run the tests: `test_eval.py` pins the set's balance (15 per verdict, at least 8 guardrails, none of them `google`). Do not continue until the user says the labels are approved.

- [ ] **Step 4: Measure the baseline (no model needed)**

Run: `python3 tools/prompt_coach/eval/run_eval.py --baseline --json tools/prompt_coach/eval/results/baseline.json`
...
for MODEL in qwen2.5:1.5b llama3.2:1b llama3.2:3b llama3:latest; do
  python3 tools/prompt_coach/eval/run_eval.py --model "$MODEL" \
      --json "tools/prompt_coach/eval/results/${MODEL//:/-}.json"
done
```
...
```bash
python3 tools/prompt_coach/eval/run_eval.py --table tools/prompt_coach/eval/results/*.json
```
...
Either way, append a `## Bake-off results` section to the spec (`docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md`) containing the date, the machine (`Apple Silicon, 24 GB` — or the actual one), the table printed above, and the recommendation line. Then commit:

```bash
git add tools/prompt_coach/eval/results tools/prompt_coach/defaults.py docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md
git commit -m "feat(prompt-coach): record bake-off results and default model"
```
...
**Files:**
- Create: `tools/prompt_coach/hook.py`, `tools/prompt_coach/launcher.py`
- Create: `tests/prompt_coach/conftest.py`, `tests/prompt_coach/test_hook.py`
...
- Produces:
  ...
  - `launcher.py` — installed as `<scope>/prompt-coach/hook.py`; puts its own folder on `sys.path`, calls `hook.main`, and swallows every exception
  - test fixture `installed_root(tmp_path)` — a Coach install laid out exactly as the wizard lays it out
...
**`tests/prompt_coach/conftest.py`**
...
def installed_root(tmp_path):
    """A Prompt Coach install laid out exactly as the setup wizard lays it out."""
    root = tmp_path / "prompt-coach"
    root.mkdir()
    shutil.copytree(TOOLS / "prompt_coach", root / "prompt_coach", ignore=shutil.ignore_patterns("__pycache__", "eval"))
    shutil.copytree(TOOLS / "token_optimizer", root / "token_optimizer", ignore=shutil.ignore_patterns("__pycache__"))
    shutil.copy(TOOLS / "prompt_coach" / "launcher.py", root / "hook.py")
    return root
````

**`tests/prompt_coach/test_hook.py`**
...
from prompt_coach import hook
from prompt_coach.errors import ModelUnavailable

from .fake_ollama import FakeOllama

COVERS = ["R1", "R3", "R4", "R6", "R7", "R8", "R11"]
...
def test_tier1_google_speaks_to_the_user(tmp_path):
    ...
    assert out == {"systemMessage": 'Prompt Coach: quick lookup — try Google: "what is the capital of France"'}
...
@pytest.mark.parametrize("env,cfg", [({"PROMPT_COACH": "off"}, {}), ({"PROMPT_COACH": " OFF "}, {}), ({}, {"enabled": False})])
...
def test_the_log_has_no_prompt_text_by_default(tmp_path):
    ...
    log = (tmp_path / "coach.log").read_text(encoding="utf-8")
...
def test_the_log_includes_the_prompt_only_when_asked(tmp_path):
    ...
    call(tmp_path, "what is the capital of Zanzibarland")
    assert "Zanzibarland" in (tmp_path / "coach.log").read_text(encoding="utf-8")
...
def test_main_exits_zero_and_prints_nothing_even_if_run_explodes(tmp_path, monkeypatch, capsys):
    ...
    assert "RuntimeError" in (tmp_path / "coach.log").read_text(encoding="utf-8")


# ---------- the real contract: run the launcher as Claude Code does --------------------------


def run_launcher(root, stdin, env_extra=None):
    env = {k: v for k, v in os.environ.items() if k != "PROMPT_COACH"}
    env.update(env_extra or {})
    return subprocess.run(
        [sys.executable, str(root / "hook.py")], input=stdin, capture_output=True, env=env, cwd=str(root.parent), timeout=60
    )
...
def test_launcher_honours_the_environment_kill_switch(installed_root):
    done = run_launcher(installed_root, event("what is the capital of France").encode(), {"PROMPT_COACH": "off"})
...
def test_a_broken_install_still_exits_zero_silently(installed_root):
    ...
    shutil.rmtree(installed_root / "prompt_coach")
    done = run_launcher(installed_root, event("what is the capital of France").encode())
    assert (done.returncode, done.stdout) == (0, b"")
````

Run: `python3 -m pytest tests/prompt_coach/test_hook.py -q`

Expected: **FAIL** — `cannot import name 'hook' from 'prompt_coach'`

- [ ] **Step 2: Hook: implementation**

**`tools/prompt_coach/hook.py`**
...
def _log(root: str, line: Dict[str, Any]) -> None:
    path = os.path.join(root, "coach.log")
    try:
...
def run(
    ...
    if env.get("PROMPT_COACH", "").strip().lower() == "off":
        return None
...
def main(root: str) -> None:
    ...
    try:
        ...
        if out is not None:
            ...
            sys.stdout.flush()
    except Exception:  # fail open by design: the Coach must never get in the way
        try:
...
**`tools/prompt_coach/launcher.py`**

````python
#!/usr/bin/env python3
"""Prompt Coach launcher. Installed as <scope>/prompt-coach/hook.py by the setup wizard.

Claude Code runs this file. It puts its own folder on the import path (so the bundled
`prompt_coach` and `token_optimizer` packages are found) and hands over to prompt_coach.hook.
...
try:
    from prompt_coach.hook import main
...
```bash
git add tools/prompt_coach/hook.py tools/prompt_coach/launcher.py tests/prompt_coach/conftest.py tests/prompt_coach/test_hook.py
git commit -m "feat(prompt-coach): add hook entry point and launcher"
```
...
**Files:**
- Create: `tools/prompt_coach/settings_merge.py`, `tools/prompt_coach/setup.py`
- Create: `tests/prompt_coach/test_settings_merge.py`, `tests/prompt_coach/test_setup.py`
...
- Produces:
  - `settings_merge.EVENT = "UserPromptSubmit"`, `MARKER = "prompt-coach/hook.py"`, `SettingsError`
...
**`tests/prompt_coach/test_settings_merge.py`**
...
from prompt_coach import settings_merge as sm

COVERS = ["R10", "R11"]

CMD = 'python3 "/home/u/.claude/prompt-coach/hook.py"'
...
def test_reinstalling_from_a_new_location_replaces_the_old_entry():
    moved = 'python3 "/elsewhere/.claude/prompt-coach/hook.py"'
...
def test_windows_style_paths_are_recognised():
    win = 'python3 "C:\\Users\\u\\.claude\\prompt-coach\\hook.py"'
    assert sm.has_hook(sm.add_hook({}, win))
    assert sm.remove_hook(sm.add_hook({}, win)) == {}


def test_remove_deletes_only_the_coach_and_restores_the_original():
...
def test_diff_shows_exactly_the_added_hook():
    ...
    assert "+" in text and "prompt-coach/hook.py" in text and "(before)" in text
    assert sm.diff_text({"a": 1}, {"a": 1}) == ""
````

Run: `python3 -m pytest tests/prompt_coach/test_settings_merge.py -q`

Expected: **FAIL** — `cannot import name 'settings_merge' from 'prompt_coach'`

- [ ] **Step 2: Settings merge: implementation**

**`tools/prompt_coach/settings_merge.py`**

````python
"""Safe, idempotent edits of a Claude Code settings file for the Prompt Coach hook.
...
def _strip(groups: List[Any]) -> List[Any]:
    """Remove the Coach's handler from every matcher group; drop groups that only held it."""
    kept: List[Any] = []
    for group in groups:
...
def add_hook(settings: Settings, command: str, timeout: int = 5) -> Settings:
    """Return a copy of `settings` with exactly one Coach handler (any older one is replaced)."""
...
def remove_hook(settings: Settings) -> Settings:
    """Return a copy of `settings` without the Coach handler, tidying any container it empties."""
    groups = _strip(_groups(settings))
    result = copy.deepcopy(settings)
    if groups:
...
```bash
git add tools/prompt_coach/settings_merge.py tests/prompt_coach/test_settings_merge.py
git commit -m "feat(prompt-coach): add safe settings merge"
```
...
**`tests/prompt_coach/test_setup.py`**
...
from prompt_coach import defaults, setup
...
def test_declining_the_first_question_changes_nothing(env, tmp_path):
    ...
    assert io.asked == ["Set up the Prompt Coach?"] and "Nothing was installed" in io.text
...
def test_dry_run_writes_nothing_and_shows_the_diff(env, tmp_path):
    ...
    assert snapshot(tmp_path) == before
    assert admin.pulled == [] and admin.started == 0
    assert "Dry run" in io.text and "+" in io.text and "prompt-coach/hook.py" in io.text
...
def test_install_local_writes_the_layout_and_one_hook_entry(env):
    ...
    root = env[1] / ".claude" / "prompt-coach"
    assert {p.name for p in root.iterdir()} == {"hook.py", "prompt_coach", "token_optimizer", "config.json", ".gitignore"}
...
def test_copy_files_ships_only_what_the_hook_needs(tmp_path):
    source_cache = setup.TOOLS_DIR / "prompt_coach" / "__pycache__"
    ...
    assert {"hook.py", "prompt_coach/decide.py", "prompt_coach/hook.py", "token_optimizer/analyzer.py", ".gitignore"} <= names
    assert not [n for n in names if "__pycache__" in n or n.endswith(".pyc")]
    assert "prompt_coach/eval" not in names and "prompt_coach/setup.py" not in names
...
def test_user_scope_installs_under_the_home_directory(env):
    ...
    assert (env[0] / ".claude" / "prompt-coach" / "hook.py").exists()
...
def test_invalid_settings_json_stops_before_any_change(env):
    ...
    assert not (path.parent / "prompt-coach").exists()
...
def test_installing_twice_keeps_one_entry_and_the_users_config_edits(env):
    ...
    config = env[1] / ".claude" / "prompt-coach" / "config.json"
...
def test_the_installed_hook_really_answers_as_claude_code_would_call_it(env):
    setup.main(flags(env, "--yes", "--scope", "local", "--no-model"), io=ScriptedIO(), ollama=FakeAdmin())
    hook = env[1] / ".claude" / "prompt-coach" / "hook.py"
...
def test_the_self_test_reports_and_leaves_no_state_behind(env):
    ...
    setup.main(flags(env, "--yes", "--scope", "local", "--no-model"), io=io, ollama=FakeAdmin())
    root = env[1] / ".claude" / "prompt-coach"
    assert not (root / "state.json").exists() and not (root / "coach.log").exists()
...
def config_of(env, scope="local"):
    ...
    return json.loads((base / ".claude" / "prompt-coach" / "config.json").read_text())
...
def test_remove_deletes_only_the_coach_and_keeps_other_hooks(env):
    path = local_settings(env)
    path.parent.mkdir(parents=True)
    original = {"model": "sonnet", "hooks": {"UserPromptSubmit": [{"hooks": [{"type": "command", "command": "lint.sh"}]}]}}
    path.write_text(json.dumps(original), encoding="utf-8")
...
def test_remove_without_a_scope_finds_either_install(env):
    ...
    assert not (env[0] / ".claude" / "prompt-coach").exists()
...
def test_update_refreshes_the_code_and_keeps_the_config(env):
    ...
    (root / "prompt_coach" / "decide.py").write_text("# corrupted\n", encoding="utf-8")
    assert setup.main(flags(env, "--update", "--scope", "local"), io=ScriptedIO(), ollama=FakeAdmin()) == 0
    assert "corrupted" not in (root / "prompt_coach" / "decide.py").read_text()
...
Run: `python3 -m pytest tests/prompt_coach/test_setup.py -q`

Expected: **FAIL** — `cannot import name 'setup' from 'prompt_coach'`

- [ ] **Step 4: Wizard: implementation**

**`tools/prompt_coach/setup.py`**

````python
"""Optional setup wizard for the Prompt Coach hook. Nothing happens without a yes.

    python3 tools/prompt_coach/setup.py                 interactive
    python3 tools/prompt_coach/setup.py --dry-run       show exactly what would change
    python3 tools/prompt_coach/setup.py --remove        undo the install

Run as a script, or import it as `prompt_coach.setup` with `tools/` on sys.path.
"""
import argparse
import copy
import json
...
if __package__ in (None, ""):  # started as a script: make `prompt_coach` importable
    sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from prompt_coach import __version__, defaults, settings_merge as sm  # noqa: E402
from prompt_coach.config import DEFAULTS, is_loopback  # noqa: E402
from prompt_coach.ollama_client import open_no_proxy  # noqa: E402

PACKAGE_DIR = Path(__file__).resolve().parent
TOOLS_DIR = PACKAGE_DIR.parent
INSTALL_NAME = "prompt-coach"
SELF_TEST_PROMPTS = (
...
def copy_files(install_dir: Path) -> None:
    ...
    for name in ("prompt_coach", "token_optimizer"):
...
def _choose_model(args: argparse.Namespace, io: Any, ollama: Any) -> ModelPlan:
    ...
    return plans[io.choose("Which model should the Coach use?", options, default)]
...
def _show_plan(io: Any, install_dir: Path, settings_path: Path, plan: ModelPlan, before: sm.Settings, after: sm.Settings) -> None:
    io.say("")
    io.say("This will:")
    io.say("  - copy the Coach into %s" % install_dir)
...
def _self_test(install_dir: Path, io: Any) -> bool:
    ...
    state, log = install_dir / "state.json", install_dir / "coach.log"
...
def _install(args: argparse.Namespace, io: Any, ollama: Any, home: str, project: str) -> int:
    io.say(INTRO)
    io.say("")
    if not args.yes and not io.ask_yes_no("Set up the Prompt Coach?", False):
    ...
    if not _self_test(install_dir, io):
        ...
        return 3
    io.say("")
    io.say("Done. Restart Claude Code so it loads the hook. Turn it off any time with PROMPT_COACH=off.")
    return 0
...
def _remove(args: argparse.Namespace, io: Any, home: str, project: str) -> int:
    ...
    if not found:
        io.say("Nothing to remove: no Prompt Coach install found.")
        return 0
...
def _update(args: argparse.Namespace, io: Any, home: str, project: str) -> int:
    ...
    if not targets:
        io.say("Nothing to update: no Prompt Coach install found.")
        return 0
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optional setup for the Prompt Coach hook.")
    ...
    parser.add_argument("--remove", action="store_true", help="remove the Coach")
...
Expected: **PASS** — `243 passed`

```bash
git add tools/prompt_coach/setup.py tests/prompt_coach/test_setup.py
git commit -m "feat(prompt-coach): add opt-in setup wizard"
```
...
- Create: `tests/prompt_coach/test_exporter_offer.py`

**Interfaces:**
- Consumes: `setup.py` as a runnable script (Task 6)
- Produces:
  - `interactive_exporter.offer_prompt_coach(project_root: Path) -> None` — prints the offer, asks `(y/N)`, and on yes runs `python3 tools/prompt_coach/setup.py --project <root>`

- [ ] **Step 1: Offer: tests first**

**`tests/prompt_coach/test_exporter_offer.py`**
...
def test_anything_but_yes_installs_nothing(monkeypatch, spy, capsys, reply):
    ...
    ie.offer_prompt_coach(Path("/tmp/proj"))
    assert spy == []
...
def test_yes_hands_over_to_the_wizard_for_this_project(monkeypatch, spy, reply):
    ...
    assert Path(command[1]) == TOOLS / "prompt_coach" / "setup.py" and Path(command[1]).exists()
...
def stub_main(monkeypatch, confirm=True, export=True):
    ...
    monkeypatch.setattr(ie, "offer_prompt_coach", lambda root: calls.append(("offer", root)))
    return calls
...
def test_the_plain_non_interactive_exporter_never_mentions_the_coach():
    # R1: a plain `exporter.py` run must never ask about, or install, the hook.
    assert "prompt_coach" not in (TOOLS / "exporter.py").read_text(encoding="utf-8")
````

Run: `python3 -m pytest tests/prompt_coach/test_exporter_offer.py -q`

Expected: **FAIL** — `no attribute 'offer_prompt_coach'` (14 failed, 1 passed: only the static "plain exporter never mentions the Coach" test passes before the change).

- [ ] **Step 2: Offer: implementation**

`exporter.py --interactive` delegates to `interactive_exporter.py`, so this is the only place to ask. The plain `exporter.py` is not touched, and a test asserts it never mentions the Coach (R1).
...
````text
def offer_prompt_coach(project_root: Path) -> None:
    """Offer the optional Prompt Coach hook. The default is No, and declining changes nothing."""
    print(f"\n{Colors.BOLD}Optional: Prompt Coach{Colors.ENDC}")
    ...
    if answer not in ("y", "yes"):
        print(f"  Skipped. Run it any time: {Colors.OKCYAN}python3 tools/prompt_coach/setup.py{Colors.ENDC}\n")
        return
    setup_script = Path(__file__).parent / "prompt_coach" / "setup.py"
...
````text
        ...
        # Step 7: Offer the optional Prompt Coach hook (default: no)
        offer_prompt_coach(project_root)
````
...
```bash
git add tools/interactive_exporter.py tests/prompt_coach/test_exporter_offer.py
git commit -m "feat: offer the Prompt Coach at the end of interactive export"
```
...
**Files:**
- Create: `docs/03-guides/prompt-coach.md`
- Modify: `README.md`, `docs/03-guides/README.md`
- Create: `tests/prompt_coach/test_docs.py`, `tests/prompt_coach/test_traceability.py`
...
**`tests/prompt_coach/test_docs.py`**
...
from prompt_coach.config import DEFAULTS
from prompt_coach.decide import Decision
from prompt_coach.output import build_output

COVERS = ["R13"]

ROOT = Path(__file__).resolve().parents[2]
GUIDE = ROOT / "docs" / "03-guides" / "prompt-coach.md"
...
def test_the_guide_covers_the_verdicts_and_the_ways_to_turn_it_off_or_undo_it():
    guide = GUIDE.read_text(encoding="utf-8")
    for needle in ("`google`", "`clarify`", "`refine`", "`pass`", "PROMPT_COACH=off", "--remove", "--dry-run", "--update", '"enabled": false'):
...
def test_the_readme_and_the_guides_index_link_to_the_guide():
    ...
    assert "(docs/03-guides/prompt-coach.md)" in README.read_text(encoding="utf-8")
    assert "(prompt-coach.md)" in GUIDES_INDEX.read_text(encoding="utf-8")
    assert GUIDE.exists()
````

Run: `python3 -m pytest tests/prompt_coach/test_docs.py -q`

Expected: **FAIL** — `assert GUIDE.exists()`

- [ ] **Step 2: Docs: the guide and the two links**

**`docs/03-guides/prompt-coach.md`**

````markdown
# Prompt Coach

An **optional** Claude Code hook that looks at each prompt before Claude does. It can tell you a
...
Design and decisions: [the design spec](../superpowers/specs/2026-09-20-prompt-coach-hook-design.md).

## What you will see

| Verdict | You see | Claude sees |
|---|---|---|
| `google` — a search would answer it | `Prompt Coach: quick lookup — try Google: "python reverse list"` | nothing; the prompt goes through |
| `clarify` — too vague to act on | `Prompt Coach: this may be too vague to act on. Missing: which file; the expected outcome` | the same gaps, so it asks you |
...
```bash
python3 tools/prompt_coach/setup.py            # interactive
python3 tools/prompt_coach/setup.py --dry-run  # show exactly what would change; writes nothing
```
...
overwritten), and a `prompt-coach/` folder next to it.
...
If the model is slow, missing, or returns something unusable, the Coach falls back to heuristics-only,
...
`config.json` sits in the `prompt-coach/` folder. Missing or invalid values fall back to the defaults.

| Key | Default | Meaning |
|---|---|---|
| `enabled` | `true` | master switch (`PROMPT_COACH=off` in the environment also disables it) |
...
- Prompts go only to a model on this machine. A non-loopback `ollama_host` is refused unless you set
  ...
  through a proxy.
- The log (`coach.log`, capped at 1 MB) records the verdict, tier, confidence and timing, never the prompt
...
```bash
PROMPT_COACH=off claude                          # off for one run
python3 tools/prompt_coach/setup.py --update     # refresh the installed code, keep your config
python3 tools/prompt_coach/setup.py --remove     # remove the hook entry and the folder
```
...
| Symptom | Check |
|---|---|
| No advice ever appears | Restart Claude Code after setup; check `PROMPT_COACH` is not `off`; look at the newest lines of `prompt-coach/coach.log` |
...
`tools/prompt_coach/eval/` holds a 60-prompt labeled set and a runner. It scores the whole pipeline on
accuracy, on the false-`google` rate (telling you to search for something that needed your code), and on
latency.

```bash
python3 tools/prompt_coach/eval/run_eval.py --baseline           # heuristics only, no model needed
python3 tools/prompt_coach/eval/run_eval.py --model <ollama-model>
```
...
````text
| **Tools** | 22 Python utilities, including the exporter | [`tools/`](tools/) · [reference](docs/02-reference/tools.md) |
| **Prompt Coach** | Optional hook that tells you when a web search would do and sharpens vague requests, using a small local model | [guide](docs/03-guides/prompt-coach.md) |
````
...
````text
| [requirement-input.md](requirement-input.md) | Requirement parsing from free text, Jira, and files | [03 — Feature from a requirement](../01-workflows/03-feature-from-requirement.md) |
| [prompt-coach.md](prompt-coach.md) | The optional Prompt Coach hook: setup, models, configuration, privacy | [14 — Export to platforms](../01-workflows/14-export-to-platforms.md) |
````
...
**`tests/prompt_coach/test_traceability.py`**
...
SPEC = HERE.parents[1] / "docs" / "superpowers" / "specs" / "2026-09-20-prompt-coach-hook-design.md"
...
def test_the_spec_lists_requirements():
    assert len(spec_requirements()) == 13
...
Run: `python3 -m pytest tests/prompt_coach/test_traceability.py -q`

Expected: **PASS** — `4 passed`

```bash
git add docs/03-guides/prompt-coach.md README.md docs/03-guides/README.md tests/prompt_coach/test_docs.py tests/prompt_coach/test_traceability.py
git commit -m "docs: add Prompt Coach guide and traceability check"
```
...
Expected: **PASS** — `500 passed` with the same `4 failed` and `41 errors` as the baseline — 233 + 267 = 500, and **no new failure**.
...
mkdir -p /tmp/coach-e2e/proj /tmp/coach-e2e/home
python3 tools/prompt_coach/setup.py --yes --scope local --no-model \
    --project /tmp/coach-e2e/proj --home /tmp/coach-e2e/home

# 2. Non-interactive run through the real Claude Code (a lookup prompt should produce the Google advice)
cd /tmp/coach-e2e/proj
claude -p "what is the capital of France" --model haiku --output-format stream-json --verbose \
    --settings /tmp/coach-e2e/proj/.claude/settings.local.json

# 3. Interactive, in your own terminal: run `claude` in /tmp/coach-e2e/proj, accept the trust prompt, and send
#    "what is the capital of France". Do you SEE a line starting "Prompt Coach: quick lookup"?   (answers O2)
...
```bash
git add docs/superpowers/specs/2026-09-20-prompt-coach-hook-design.md
git commit -m "docs: record Prompt Coach end-to-end findings"
```

- [ ] **Step 6: Finish the branch**

**REQUIRED SUB-SKILL:** use `superpowers:finishing-a-development-branch` to decide how to integrate `feat/prompt-coach`. It has never been pushed; do not push without the user's say-so.
...
|---|---|
| **R1** — Opt-in | `test_exporter_offer.py`, `test_hook.py`, `test_setup.py` |
